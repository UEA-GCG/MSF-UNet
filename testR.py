...
if __name__ == '__main__':
    ...
    dataset = RGBDD_Dataset(root_dir='./data/SRData/RGBDD', scale=args.scale[-1], transform=data_transform)
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            guidance, lr, gt, max, min = data['guidance'].to(device), data['lr'].to(device), data['gt'].to(device), data[
                        'max'].to(device), data['min'].to(device)
            out = net(lr, guidance)

            minmax = [max, min]

            # img_grid = torch.cat((out, gt), -1)
            # utils.save_image(img_grid, f"./test/RDCA7.0/RGBDD/RGBDD_X{args.scale[-1]}_{idx}.png", nrow=1, normalize=False)
            ...
            cv2.imwrite(f"./test/RDCA6.0/RGBDD/RGBDD_X{args.scale[-1]}_{idx}Bic.png", lr_bic)
            cv2.imwrite(f"./test/RDCA6.0/RGBDD/RGBDD_X{args.scale[-1]}_{idx}depth.png", out_n)
            cv2.imwrite(f"./test/RDCA6.0/RGBDD/RGBDD_X{args.scale[-1]}_{idx}GT.png", gt_n)

            utils.save_image(guidance, f"./test/RDCA6.0/RGBDD/RGBDD_X{args.scale[-1]}_{idx}RGB.png", nrow=1,
                             normalize=False)
            # rmse[idx] = rgbdd_calc_rmse(gt[0,0], out[0,0])
            rmse[idx] = rgbdd_calc_rmse(gt[0, 0], out[0, 0], minmax)
        print(f"RGBDD_X{args.scale[-1]} 平均RMSE：{rmse.mean()}")
