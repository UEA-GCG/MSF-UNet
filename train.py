...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', type=int, default=args.scale[-1], help='scale factor')
    ...
    parser.add_argument("--decay_iterations", type=list, default=[1e5, 2e5], help="steps to start lr decay")
    ...
    net = MSFUNet.make_model(args).cuda()
    criterion = nn.L1Loss()
    ...
    for epoch in range(1, max_epoch+1):
        ...
        logging.info('epoch:%d iteration:%d running_loss:%.10f' % (epoch, batches_done, running_loss/num_train))
        ...
        if epoch % 2 == 0:
            with torch.no_grad():
                ...
                logging.info('---------------------------------------------------------------------------------------------------------------------------')
                logging.info('epoch:%d lr:%f-------mean_rmse:%f (BEST: %f @epoch%d)'%(epoch, scheduler.get_last_lr()[0], r_mean, best_rmse, best_epoch))
                logging.info('---------------------------------------------------------------------------------------------------------------------------')
