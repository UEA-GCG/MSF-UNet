...
from data.Lu_dataloader import *
...
if __name__ == '__main__':
    ...
    dataset = Lu_Dataset(root_dir='./data/SRData/Lu', scale=args.scale[-1], transform=data_transform)
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            cv2.imwrite(f"./test/RDCA6.0/Lu/Lu_X{args.scale[-1]}_{idx}Bic.png", lr_bic)
            cv2.imwrite(f"./test/RDCA6.0/Lu/Lu_X{args.scale[-1]}_{idx}depth.png", out_n)
            cv2.imwrite(f"./test/RDCA6.0/Lu/Lu_X{args.scale[-1]}_{idx}GT.png", gt_n)

            utils.save_image(guidance, f"./test/RDCA6.0/Lu/Lu_X{args.scale[-1]}_{idx}RGB.png", nrow=1,
                             normalize=False)

            # img_grid = torch.cat((out, gt), -1)
            # utils.save_image(img_grid, f"./test/RDCA6.0/Lu/Lu_X{args.scale[-1]}_{idx}.png", nrow=1, normalize=False)
            rmse[idx] = midd_calc_rmse(gt[0,0], out[0,0])
        print(f"Lu_X{args.scale[-1]} 平均RMSE：{rmse.mean()}")
