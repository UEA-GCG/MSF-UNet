...
if __name__ == '__main__':
    ...
    dataset = NYU_v2_datset(root_dir='./data/SRData/NYU_v2', scale=args.scale[-1], transform=data_transform, train=False)
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            # utils.save_image(img_grid, f"./test/NYUv2_X{args.scale[-1]}_{idx}.png", nrow=1, normalize=False)
            ...
            cv2.imwrite(f"./test/RDCA6.0/NYUv2/NYUv2_X{args.scale[-1]}_{idx}Bic.png", lr_bic)
            cv2.imwrite(f"./test/RDCA6.0/NYUv2/NYUv2_X{args.scale[-1]}_{idx}depth.png", out_n)
            cv2.imwrite(f"./test/RDCA6.0/NYUv2/NYUv2_X{args.scale[-1]}_{idx}GT.png", gt_n)

            utils.save_image(guidance, f"./test/RDCA6.0/NYUv2/NYUv2_X{args.scale[-1]}_{idx}RGB.png", nrow=1,
                             normalize=False)

            minmax = test_minmax[:,idx]
            minmax = torch.from_numpy(minmax).to(device)
            rmse[idx] = calc_rmse(gt[0,0], out[0,0], minmax=minmax)
        print(f"NYUv2_X{args.scale[-1]} 平均RMSE：{rmse.mean()}")
